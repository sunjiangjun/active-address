...
const en = {
    ...
    gasReserveHint:
      "Each transaction randomly picks one private key. The chosen account must keep {reserve} QDAY after the transfer.",
    lowBalance: "Balance is below {reserve} QDAY",
    addPrivateKey: "Add private key",
    removePrivateKey: "Remove private key",
    sender: "From",
    maxSpend: "Max QDAY to spend",
    addressCount: "Recipient count",
    specifiedAddresses: "Specified recipients (optional, one address per line)",
    specifiedHint:
      "One address per line. If there are fewer addresses than the required count, a 12-word mnemonic is generated and the missing addresses are added to this box. Each amount is random within the max spend; all amounts sum to the max spend. Derivation path: {path}.",
    ...
    senderInsufficient:
      "Sender {address} has {have} QDAY, needs {need} QDAY for this tx (amount + {reserve} QDAY gas reserve)",
...
const zh: { [K in keyof typeof en]: string } = {
    ...
    senderAddress: "发送地址",
    currentBalance: "当前余额",
    gasReserveHint: "Gas 预留 {reserve} QDAY。被选中的发送账户在转账后需至少剩余 {reserve} QDAY。",
    lowBalance: "余额低于 {reserve} QDAY",
    addPrivateKey: "添加私钥",
    removePrivateKey: "删除私钥",
    sender: "发送方",
    maxSpend: "最大花费 QDAY 数额",
    addressCount: "接收地址数量",
    specifiedAddresses: "指定接收地址（可选，每行一个）",
    specifiedHint:
      "每行一个地址。若数量不足，将自动生成 12 词助记词，并把补齐的地址追加到此输入框。每笔金额在最大花费额度内随机生成，总额等于最大花费。派生路径：{path}。",
    prepare: "生成地址并校验",
    preparing: "校验中…",
    start: "开始发送",
    stop: "停止发送",
    ...
    senderInsufficient:
      "发送账户 {address} 余额 {have} QDAY，本笔需要 {need} QDAY（金额 + 预留 {reserve} QDAY）",
